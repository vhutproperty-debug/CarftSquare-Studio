...
import { reconnectPortal } from '@/lib/research/browser-gateway/gateway';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await reconnectPortal({
      workspaceId,
      portal,
    ...
    return NextResponse.json({
      ok: true,
      ...result,
      message: 'Previous session cleared. New remote browser login queued.',
    });
  } catch (error) {
    console.error('[research] reconnect_failed', error);
    return NextResponse.json(
