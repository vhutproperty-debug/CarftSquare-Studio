...
import type { OpsDemandActivity } from '@/lib/ops/demand/types';
import { DEMAND_ACTIVITY_LABELS } from '@/lib/ops/demand/statuses';
...
export default function DemandActivityTimeline({ activities }: { activities: OpsDemandActivity[] }) {
        ...
        <li key={activity.id} className="rounded-lg border border-slate-100 bg-slate-50 px-3 py-2.5">
          <div className="flex flex-wrap items-baseline justify-between gap-2">
            <p className="text-xs font-bold uppercase tracking-wide text-slate-500">
              {DEMAND_ACTIVITY_LABELS[activity.type]}
            </p>
