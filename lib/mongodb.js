...
export function getMongoUrl() {
  return String(process.env.MONGODB_URI || process.env.DATABASE_URL || process.env.MONGO_URL || '').trim();
}
...
async function connectOnce() {
  ...
  await cachedDb.collection('admins').createIndex({ role: 1, status: 1 });
  await cachedDb.collection('admin_activity_logs').createIndex({ id: 1 }, { unique: true });
  await cachedDb.collection('admin_activity_logs').createIndex({ createdAt: -1 });
  await cachedDb.collection('admin_activity_logs').createIndex({ actorId: 1, createdAt: -1 });
  await cachedDb.collection('admin_activity_logs').createIndex({ action: 1, createdAt: -1 });
  ...
  await cachedDb.collection('designer_callback_leads').createIndex({ id: 1 }, { unique: true });
  await cachedDb.collection('designer_callback_leads').createIndex({ createdAt: -1 });
  await cachedDb.collection('designer_callback_leads').createIndex({ status: 1 });
  await cachedDb.collection('designer_callback_leads').createIndex({ phone: 1 });
  await cachedDb.collection('customer_reviews').createIndex({ id: 1 }, { unique: true });
  await cachedDb.collection('customer_reviews').createIndex({ status: 1 });
  await cachedDb.collection('customer_reviews').createIndex({ createdAt: -1 });
  await cachedDb.collection('quotation_consultations').createIndex({ id: 1 }, { unique: true });
  await cachedDb.collection('quotation_consultations').createIndex({ createdAt: -1 });
  await cachedDb.collection('blog_posts').createIndex({ id: 1 }, { unique: true });
  await cachedDb.collection('blog_posts').createIndex({ slug: 1 }, { unique: true });
  await cachedDb.collection('blog_posts').createIndex({ status: 1, publishedAt: -1 });
  await cachedDb.collection('blog_posts').createIndex({ status: 1, category: 1, publishedAt: -1 });
  await cachedDb.collection('blog_posts').createIndex({ status: 1, tags: 1, publishedAt: -1 });

  const { ensurePartnerNetworkIndexes } = await import('@/lib/partner-network/store');
  await ensurePartnerNetworkIndexes(cachedDb);
...
/** True when the process already holds a connected Mongo client + db (warm isolate). */
