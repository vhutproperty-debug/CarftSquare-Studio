...
import type { OpsMatchActivity } from '@/lib/ops/matching/types';
...
export default function MatchTimeline({ activities }: { activities: OpsMatchActivity[] }) {
        ...
        <li key={activity.id} className="rounded-lg border border-slate-100 bg-slate-50 px-3 py-2.5">
          <div className="flex flex-wrap items-baseline justify-between gap-2">
            <p className="text-xs font-bold uppercase tracking-wide text-slate-500">
              {MATCH_ACTIVITY_LABELS[activity.type]}
            </p>
