...
import { disconnectPortal } from '@/lib/research/browser-gateway/gateway';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ok: true,
      message: 'Disconnected. Encrypted session and browser profile removed.',
    });
  } catch (error) {
    console.error('[research] disconnect_failed', error);
    return NextResponse.json(
